...
class _WorkoutSessionScreenState extends State<WorkoutSessionScreen> {
  ...
  String _getImagePath(String exerciseName) {
    ...
    String nameForAlias = cleanName.replaceAll(RegExp(r'[áàâã]'), 'a')
        .replaceAll(RegExp(r'[éèê]'), 'e').replaceAll(RegExp(r'[íìî]'), 'i')
        .replaceAll(RegExp(r'[óòôõ]'), 'o').replaceAll(RegExp(r'[úùû]'), 'u')
        .replaceAll('ç', 'c');

    if (aliases.containsKey(nameForAlias)) return 'assets/images/${aliases[nameForAlias]}.gif';
  ...
  void _initializeSets() {
    ...
    for (int i = 0; i < exercises.length; i++) {
      if (!SessionStateCache.setsStatus.containsKey(i)) {
        ...
        SessionStateCache.setsStatus[i] = List.generate(setsCount, (_) => false);
      ...
      if (!_weightControllers.containsKey(i)) {
        ...
        for (int j = 0; j < SessionStateCache.setsStatus[i]!.length; j++) {
          var wCtrl = TextEditingController(text: SessionStateCache.weights[i]![j]);
          wCtrl.addListener(() { SessionStateCache.weights[i]![j] = wCtrl.text; });
          _weightControllers[i]!.add(wCtrl);

          var rCtrl = TextEditingController(text: SessionStateCache.reps[i]![j]);
          rCtrl.addListener(() { SessionStateCache.reps[i]![j] = rCtrl.text; });
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            const Text('Escolha seu aplicativo de áudio 🎧', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white)),
            const SizedBox(height: 16),
            ListTile(
              leading: const Icon(Icons.play_circle_filled, color: Colors.redAccent, size: 28),
              title: const Text('YouTube Music', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              onTap: () {
                Navigator.pop(ctx);
                _openMusicApp('vnd.youtube.music://', 'https://music.youtube.com');
              },
            ),
            ListTile(
              leading: const Icon(Icons.music_note, color: Colors.green, size: 28),
            ...
            ListTile(
              leading: const Icon(Icons.graphic_eq, color: Colors.cyanAccent, size: 28),
              title: const Text('Deezer', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              onTap: () {
                Navigator.pop(ctx);
            ...
            ListTile(
              leading: const Icon(Icons.apple, color: Colors.pinkAccent, size: 28),
              title: const Text('Apple Music', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              onTap: () {
                Navigator.pop(ctx);
            ...
            ListTile(
              leading: const Icon(Icons.queue_music, color: Colors.orangeAccent, size: 28),
              title: const Text('Amazon Music', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              onTap: () {
                Navigator.pop(ctx);
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Erro ao abrir o reprodutor de música.')),
        );
      }
    }
  }
  ...
  void _confirmExit(WorkoutProvider provider) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: Theme.of(context).colorScheme.surface,
        title: const Text('Pausar ou Encerrar?', style: TextStyle(fontWeight: FontWeight.w800, color: Colors.white)),
        content: const Text('Deseja minimizar para continuar depois ou encerrar definitivamente e descartar o progresso de hoje?', style: TextStyle(color: AppColors.textSecondary)),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(ctx);
            ...
            child: Text('Minimizar', style: TextStyle(color: Theme.of(context).colorScheme.primary)),
          ),
          ElevatedButton(
            ...
            child: const Text('Descartar Treino', style: TextStyle(fontWeight: FontWeight.w800)),
          ),
        ],
      ),
    );
        ...
        builder: (ctx) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: Colors.redAccent, foregroundColor: Colors.white),
              onPressed: () {
                provider.finishWorkout(_formatTime(provider.workoutDuration.value), isIncomplete: true, logs: workoutLogs, notes: _notesController.text);
                SessionStateCache.clear();
                Navigator.pop(ctx);
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              provider.finishWorkout(_formatTime(provider.workoutDuration.value), isIncomplete: false, logs: workoutLogs, notes: _notesController.text);
              SessionStateCache.clear();
              Navigator.pop(ctx);
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: const Text('Treino concluído e salvo com sucesso!'), backgroundColor: Theme.of(context).colorScheme.primary, duration: const Duration(seconds: 3)),
              );
            },
            child: const Text('Salvar Treino', style: TextStyle(fontWeight: FontWeight.w800)),
          ),
        ],
      ),
    );
  ...
  Widget _buildInputForm(BuildContext context, TextEditingController controller, String hint, {bool isReps = false}) {
    return SizedBox(
      height: 32,
      child: TextField(
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(provider.activeRoutineName, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
          actions: [
            IconButton(
            ...
            Container(
              ...
              child: Column(
                children: [
                  const Text(
                    'TEMPO TOTAL',
                    style: TextStyle(color: AppColors.textSecondary, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 1.0),
                  ),
              ...
              child: exercises.isEmpty
                ...
                child: ElevatedButton.icon(
                  ...
                  onPressed: () => Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const ExercisesScreen(isSelecting: true)),
                  ),
                ...
                itemBuilder: (context, index) {
                  ...
                  final bool isLinked = ex.isSuperset && index < exercises.length - 1;
                  final bool isPreviousLinked = index > 0 && exercises[index - 1].isSuperset;
                  ...
                  return Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border(
                        top: BorderSide(color: isPreviousLinked ? Colors.transparent : AppColors.border),
                        left: const BorderSide(color: AppColors.border),
                        right: const BorderSide(color: AppColors.border),
                        bottom: BorderSide(color: isLinked ? Colors.transparent : AppColors.border),
                      ),
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text('BI-SET (FAÇA JUNTO COM O ACIMA)', style: TextStyle(color: Theme.of(context).colorScheme.primary, fontSize: 11, fontWeight: FontWeight.w800)),
                              ],
                            ),
                          ),

                        InkWell(
                          borderRadius: BorderRadius.vertical(top: Radius.circular(isPreviousLinked ? 0 : 14)),
                          onTap: () {
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Center(child: Text(ex.name, textAlign: TextAlign.center, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: Colors.white))),
                                        const SizedBox(height: 4),
                                        Center(child: Text(ex.muscle.toUpperCase(), style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.w800, letterSpacing: 1.0))),
                                        const SizedBox(height: 24),
                                        const Text('MÚSCULOS SECUNDÁRIOS:', style: TextStyle(fontSize: 10, color: AppColors.textSecondary, fontWeight: FontWeight.bold)),
                                        Text(details['secondary'], style: const TextStyle(fontSize: 13, color: Colors.white)),
                                        const SizedBox(height: 16),
                                        const Text('COMO EXECUTAR:', style: TextStyle(fontSize: 10, color: AppColors.textSecondary, fontWeight: FontWeight.bold)),
                                        const SizedBox(height: 8),
                                        ...List.generate((details['steps'] as List).length, (i) {
                                          return Padding(
                                            padding: const EdgeInsets.only(bottom: 8.0),
                                            child: Row(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text('${i + 1}.', style: TextStyle(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.bold)),
                                                const SizedBox(width: 6),
                                                Expanded(child: Text(details['steps'][i], style: const TextStyle(fontSize: 13, color: Colors.white, height: 1.3))),
                                              ],
                                            ),
                                          );
                                        }),
                                        ...
                                        SizedBox(
                                          ...
                                          child: TextButton(
                                            onPressed: () => Navigator.pop(ctx),
                                            child: Text('FECHAR', style: TextStyle(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.primary)),
                                          ),
                                        )
                                      ],
                                    ),
                          ...
                          child: ListTile(
                            ...
                            title: Text(ex.name, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14)),
                            subtitle: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text('Última carga: ${_getLastWeight(ex.id)}',
                                      style: TextStyle(color: Theme.of(context).colorScheme.primary, fontSize: 11, fontWeight: FontWeight.w600)),
                                ],
                              ),
                            ),
                            trailing: const Icon(Icons.help_outline, color: AppColors.textSecondary, size: 20),
                          ),
                        ),
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: const [
                              Expanded(flex: 2, child: Text('Série', style: TextStyle(fontSize: 11, color: AppColors.textSecondary, fontWeight: FontWeight.bold))),
                              Expanded(flex: 3, child: Center(child: Text('Carga (kg)', style: TextStyle(fontSize: 11, color: AppColors.textSecondary, fontWeight: FontWeight.bold)))),
                              Expanded(flex: 3, child: Center(child: Text('Reps', style: TextStyle(fontSize: 11, color: AppColors.textSecondary, fontWeight: FontWeight.bold)))),
                              Expanded(flex: 2, child: Align(alignment: Alignment.centerRight, child: Icon(Icons.check, size: 14, color: AppColors.textSecondary))),
                            ],
                          ),
                        ),
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                          ...
                                          child: Padding(
                                            ...
                                            child: _buildInputForm(context, _weightControllers[index]![setIndex], 'kg'),
                                          )
                                      ),
                                      Expanded(
                                          flex: 3,
                ...
                children: [
                  if (provider.isResting)
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              const SizedBox(width: 8),
                              Text('DESCANSO', style: TextStyle(fontWeight: FontWeight.w800, color: Theme.of(context).colorScheme.primary, fontSize: 13)),
                            ],
                          ),
                          Text(
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text('Descanso Inteligente Ativado', style: TextStyle(color: AppColors.textSecondary, fontWeight: FontWeight.w600, fontSize: 13)),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Text('FINALIZAR E SALVAR', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, letterSpacing: 1.0)),
                    ),
                  ),
                ],
              ),
